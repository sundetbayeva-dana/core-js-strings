...
function removeFirstOccurrences(str, value) {
  const firstSearchIndex = str.indexOf(value);
  let res = '';
  if (firstSearchIndex >= 0) {
    res =
      str.slice(0, firstSearchIndex) +
      str.slice(firstSearchIndex + value.length);
  } else {
    res = str;
  }
  return res;
...
function removeLastOccurrences(str, value) {
  const lastSearchIndex = str.lastIndexOf(value);
  let res = '';
  if (lastSearchIndex >= 0) {
    res =
      str.slice(0, lastSearchIndex) + str.slice(lastSearchIndex + value.length);
  } else {
    res = str;
  }
  return res;
 ...
 */
function isPalindrome(origStr) {
  let str = '';
  for (let i = 0; i < origStr.length; i += 1) {
    if (
      origStr[i] !== ',' &&
      origStr[i] !== '?' &&
      origStr[i] !== '!' &&
      ...
      str += origStr[i].toLowerCase();
    }
  }
  const compareStr = str.split('').reverse().join('');
  return str.toLowerCase() === compareStr;
}
...
function extractNameFromTemplate(str) {
  ...
  return `${array[1]} ${array[2].slice(0, array[2].length - 1)}`;
}
...
function getCardId(value) {
  let rank = 0;
  let suits = 0;
  const suitsValue = value[value.length - 1];

  if (!Number.isNaN(parseInt(value, 10))) {
    rank = parseInt(value, 10) - 1;
  } else {
    switch (value[0]) {
      case 'A':
        rank = 0;
